...
Perform all reasoning to justify each score and report it in the relative output place.**
...
- reference program  
- requests that the program must implement
...
### STEP 1.1 - Reasoning (negative evidences only)
Basing only on the generated evidences with `"goodness": "-"`, justify the score that is gonna be assigned to the related topic.
